...
_br_scan_adb() {
    ...
    adb shell "ls -1 ${ADB_BR_DIR}/ 2>/dev/null" 2>/dev/null \
        | tr -d '\r' | grep -E '\.zip$' | sort -r | head -10
}
...
_br_pick_file() {
    ...
    # ── Recolectar de Descargas ───────────────────────────────────────────────
    local -a DL_FILES=()
    while IFS= read -r f; do [ -n "$f" ] && DL_FILES+=("$f"); done \
        < <(_br_scan_downloads)

    # ── Recolectar de carpeta interna (via ADB) ───────────────────────────────
    local -a ADB_FILES=()
    local ADB_BR_DIR="/data/user_de/0/com.android.shell/files/bugreports"
    ...
    if [ ${#DL_FILES[@]} -gt 0 ]; then
        ...
        for f in "${DL_FILES[@]}"; do
            ...
            idx=$(( ${#ALL_LABELS[@]} + 1 ))
            echo -e "${Y}  [$idx]${W} $(basename "$f")${N}"
            echo -e "      ${B}Tamanio: ${W}$SIZE   ${B}Fecha: ${W}$DATE_STR${N}"
            echo ""
    ...
    if [ ${#ADB_FILES[@]} -gt 0 ]; then
        echo -e "${M}  Informes del sistema (carpeta interna via ADB):${N}"
        echo ""
        for name in "${ADB_FILES[@]}"; do
            # Saltar si ya está en Descargas
            local already=0
            ...
            echo -e "      ${B}(se copiara a Descargas al seleccionarlo)${N}"
            echo ""
            ALL_LABELS+=("$name")
            ALL_TYPES+=("adb")
            ALL_PATHS+=("$name")
    ...
    if [ $total -gt 0 ]; then
        echo -ne "${Y}  Elegí un numero (1-${total}), R para ruta manual, V para volver: ${N}"
    else
        echo -ne "${Y}  R para ruta manual, V para volver: ${N}"
    fi
    read -r SEL
    ...
    case "${SEL^^}" in
        V) return 1 ;;
        R)
            echo -ne "${Y}  Ruta completa del .zip: ${N}"
            read -r RUTA_MANUAL
            RUTA_MANUAL=$(echo "$RUTA_MANUAL" | tr -d "'\"\r")
            if [ -f "$RUTA_MANUAL" ] && [ -s "$RUTA_MANUAL" ]; then
                BR_SELECTED_FILE="$RUTA_MANUAL"
                return 0
            fi
            echo -e "${R}  [!] Archivo no encontrado o vacio.${N}"
            return 1
            ;;
        *)
