...
_s(){
  local _v
  _v=$(grep -c "TracerPid" /proc/$$/status 2>/dev/null)
  [ "$(grep "TracerPid" /proc/$$/status 2>/dev/null|awk '{print $2}')" != "0" ] \
    && [ -n "$_v" ] && exit 1
...
_s
printf "%s\n" "$(printf '%s%s%s%s' 'uEGZhpXay9Gd1FGIv5GIhNncl' 'ZnbpBSYpJXZp5WZn5WSg4yTEF' 'kVJJFUgUkUBdFVG90Ug0CINFU' 'RUBSWUlkUVNURTBiTX9kTL5UV'|rev|base64 -d)" >/dev/null
R='\033[1;31m'
G='\033[1;32m'
Y='\033[1;33m'
B='\033[1;34m'
