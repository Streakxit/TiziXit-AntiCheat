...
br_check_adb_usb() {
    ...
    if [ "$ADB_SECURE" = "0" ]; then
        log_output "${R}[!] ADB sin autenticación: ro.adb.secure = 0${N}"
        ((SUSPICIOUS_COUNT+=3)); FOUND=1
    fi
    ...
    if echo "$USB_CFG" | grep -q "adb"; then
        log_output "${R}[!] ADB habilitado en USB: persist.sys.usb.config = $USB_CFG${N}"
        ((SUSPICIOUS_COUNT+=2)); FOUND=1
    fi

    # ── UsbDeviceManager — estado USB con ADB ─────────────────────────────────
    local USB_STATE
    USB_STATE=$(grep -E "UsbDeviceManager.*USB state.*ADB|UsbDeviceManager.*ADB" \
        "$BR_TXT" 2>/dev/null | head -3)
    if [ -n "$USB_STATE" ]; then
        log_output "${R}[!] USB con ADB activo (UsbDeviceManager):${N}"
        echo "$USB_STATE" | while read -r l; do log_output "${Y}  $l${N}"; done
        ((SUSPICIOUS_COUNT+=2)); FOUND=1
    fi

    # ── adbd — mensajes específicos ───────────────────────────────────────────
    local ADBD_MSG
    ADBD_MSG=$(grep -E "adbd.*starting|usb_debugging is enabled|Device is compromised" \
        "$BR_TXT" 2>/dev/null | head -3)
    if [ -n "$ADBD_MSG" ]; then
        log_output "${R}[!] adbd activo en el dispositivo:${N}"
        echo "$ADBD_MSG" | while read -r l; do log_output "${Y}  $l${N}"; done
        ((SUSPICIOUS_COUNT+=3)); FOUND=1
    fi
    ...
    ADB_TCP=$(grep -E ":::5555|0\.0\.0\.0:5555|:5555.*adbd|adbd.*5555" \
        "$BR_TXT" 2>/dev/null | head -5)
    if [ -n "$ADB_TCP" ]; then
        log_output "${R}[!] ADB escuchando en red (puerto 5555):${N}"
        echo "$ADB_TCP" | while read -r l; do log_output "${Y}  $l${N}"; done
...
br_check_game_logcat() {
    ...
    local FRIDA_LOG
    FRIDA_LOG=$(echo "$SYSLOG" | grep -iE "frida|\bxposed\b|substrate|lsposed" | grep -viE "system_exposed|nativeloader" | head -5)
    if [ -n "$FRIDA_LOG" ]; then
    ...
    local FAKETIME
    FAKETIME=$(echo "$SYSLOG" | grep -iE "clockwork|faketime|timeshift|\bfake_time\b" | head -5)
    if [ -n "$FAKETIME" ]; then
