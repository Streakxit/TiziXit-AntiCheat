...
# ═══════════════════════════════════════════════════════════════
# INFO DEL DISPOSITIVO — se muestra al inicio del análisis
# ═══════════════════════════════════════════════════════════════
br_get_device_info() {
    local PROPS
    PROPS=$(_br_sec "SYSTEM PROPERTIES")
    # Fallback: leer propiedades directamente del txt (formato [key]: [value])
    [ -z "$PROPS" ] && PROPS=$(grep -E "^\[" "$BR_TXT" 2>/dev/null | head -300)

    BR_DEVICE_MODEL=$(echo "$PROPS" | grep -E "\[ro\.product\.model\]"          | sed 's/.*\]: \[//;s/\]//' | head -1)
    BR_DEVICE_BRAND=$(echo "$PROPS" | grep -E "\[ro\.product\.brand\]"           | sed 's/.*\]: \[//;s/\]//' | head -1)
    BR_ANDROID_VER=$(echo "$PROPS"  | grep -E "\[ro\.build\.version\.release\]"  | sed 's/.*\]: \[//;s/\]//' | head -1)
    BR_COUNTRY_SIM=$(echo "$PROPS"  | grep -E "\[gsm\.operator\.iso-country\]"   | sed 's/.*\]: \[//;s/\]//' | head -1 | cut -d',' -f1 | tr -d ' ')
    BR_COUNTRY_SIM2=$(echo "$PROPS" | grep -E "\[gsm\.sim\.operator\.iso-country\]" | sed 's/.*\]: \[//;s/\]//' | head -1 | cut -d',' -f1 | tr -d ' ')
    BR_TIMEZONE=$(echo "$PROPS"     | grep -E "\[persist\.sys\.timezone\]"       | sed 's/.*\]: \[//;s/\]//' | head -1)
    BR_LOCALE=$(echo "$PROPS"       | grep -E "\[ro\.product\.locale\]|\[persist\.sys\.locale\]" | sed 's/.*\]: \[//;s/\]//' | head -1)
    BR_SERIAL=$(echo "$PROPS"       | grep -E "\[ro\.serialno\]|\[ro\.boot\.serialno\]" | sed 's/.*\]: \[//;s/\]//' | head -1)
    BR_SECURITY_PATCH=$(echo "$PROPS" | grep -E "\[ro\.build\.version\.security_patch\]" | sed 's/.*\]: \[//;s/\]//' | head -1)

    [ -z "$BR_COUNTRY_SIM" ] && BR_COUNTRY_SIM="$BR_COUNTRY_SIM2"
    [ -z "$BR_COUNTRY_SIM" ] && BR_COUNTRY_SIM="N/A"
}

br_show_device_header() {
    sec_hdr "INFORMACIÓN DEL DISPOSITIVO"
    local BRAND_MODEL="${BR_DEVICE_BRAND:-?} ${BR_DEVICE_MODEL:-N/A}"
    log_output "${W}  Dispositivo:     ${C}${BRAND_MODEL}${N}"
...
# ═══════════════════════════════════════════════════════════════
# ROOT — KernelSU / Magisk / Shamiko / APatch
# ═══════════════════════════════════════════════════════════════
br_check_root() {
    sec_hdr "ROOT — KernelSU / Magisk / Shamiko"
    local FOUND=0
    ...
    # ── 1. Boot state / bootloader ────────────────────────────────────────────
    local VBOOT
    VBOOT=$(echo "$PROPS" | grep -E "\[ro\.boot\.verifiedbootstate\]" | sed 's/.*\]: \[//;s/\]//')
    if [ -n "$VBOOT" ] && [ "$VBOOT" != "green" ]; then
        log_output "${R}[!] Bootloader desbloqueado: verifiedbootstate = $VBOOT${N}"
    ...
    ROOT_PKGS=(
        ...
        ["io.github.kernelsu"]="KernelSU"
        ["shirkneko.zako"]="Zako (ShamikoPro)"
        ["com.samsung.android.honeyboard"]=""   # falso positivo frecuente — ignorar
    )
    local PKGS_SEC
    PKGS_SEC=$(_br_sec "INSTALLED PACKAGES")
    for pkg in "${!ROOT_PKGS[@]}"; do
        [ -z "${ROOT_PKGS[$pkg]}" ] && continue   # ignorar entradas vacías
        if echo "$PKGS_SEC" | grep -q "$pkg"; then
            log_output "${R}[!] App root instalada: ${ROOT_PKGS[$pkg]}  (${pkg})${N}"
    ...
    # ── 3. Paths /data/adb ────────────────────────────────────────────────────
    local ROOT_PATHS
    ROOT_PATHS=$(grep -iE "/data/adb/magisk|/data/adb/ksu|/data/adb/ap[^p]|/data/adb/modules" \
        "$BR_TXT" 2>/dev/null | grep -viE "AntiCheat|com\.termux" | head -5)
    if [ -n "$ROOT_PATHS" ]; then
        log_output "${R}[!] Paths de root en el sistema:${N}"
        echo "$ROOT_PATHS" | while read -r l; do log_output "${Y}  $l${N}"; done
        ((SUSPICIOUS_COUNT+=5)); FOUND=1
    fi

    # ── 4. Logcat — procesos activos de root ──────────────────────────────────
    for kw in magiskd ksud shamiko zygisk_proc; do
        local hits
        hits=$(grep -ic "$kw" "$BR_TXT" 2>/dev/null || echo 0)
        if [ "$hits" -gt 2 ]; then
            log_output "${R}[!] Proceso root en log: ${kw} (${hits} apariciones)${N}"
            ((SUSPICIOUS_COUNT+=4)); FOUND=1
        fi
    done

    # ── 5. Propiedades de debug / spoofing ────────────────────────────────────
    local SPOOF
    SPOOF=$(echo "$PROPS" | grep -iE "ro\.product\.first_api_level|persist\.sys\.safetynet|ro\.build\.type.*userdebug" | head -3)
    local DEBUG
    DEBUG=$(echo "$PROPS" | grep -E "\[ro\.debuggable\]" | sed 's/.*\]: \[//;s/\]//')
    if [ "$DEBUG" = "1" ]; then
        log_output "${Y}[!] Build debuggable (ro.debuggable=1)${N}"
        ((SUSPICIOUS_COUNT+=2)); FOUND=1
    fi

    [ $FOUND -eq 0 ] && log_output "${G}[✓] Sin indicadores de root detectados${N}"
    log_output ""
}

# ═══════════════════════════════════════════════════════════════
# CONEXIONES ADB / USB RECIENTES (últimas 5 horas)
# ═══════════════════════════════════════════════════════════════
br_check_adb_usb() {
    sec_hdr "CONEXIONES ADB / USB RECIENTES"
    local FOUND=0

    # Calcular hora de corte: generación - 5h
    local GEN_H=99
    local BR_FNAME
    BR_FNAME=$(basename "${ZIP_PATH:-$BR_TXT}")
    if [[ "$BR_FNAME" =~ -([0-9]{2})-[0-9]{2}-[0-9]{2}\.zip$ ]]; then
        GEN_H=$(( 10#${BASH_REMATCH[1]} ))
    elif [[ "$BR_FNAME" =~ [0-9]{4}-[0-9]{2}-[0-9]{2}-([0-9]{2})- ]]; then
        GEN_H=$(( 10#${BASH_REMATCH[1]} ))
    fi
    local CUTOFF_H=$(( (GEN_H - 5 + 24) % 24 ))

    # Buscar eventos ADB en logcat filtrando por hora si fue posible extraerla
    local ADB_LINES
    if [ "$GEN_H" -ne 99 ]; then
        # Filtrar líneas con hora >= CUTOFF_H (formato MM-DD HH:)
        ADB_LINES=$(grep -iE "adbd|adb_auth|ADB_ENABLED|adb\.tcp" "$BR_TXT" 2>/dev/null \
            | awk -v cut="$CUTOFF_H" '
                match($0, /[0-9]{2}-[0-9]{2} ([0-9]{2}):/, a) {
                    h = int(a[1])
                    if (h >= cut) print
                }
                !/[0-9]{2}-[0-9]{2} [0-9]{2}:/ { print }
            ' | head -10)
    else
        ADB_LINES=$(grep -iE "adbd|adb_auth|ADB_ENABLED|adb\.tcp" "$BR_TXT" 2>/dev/null | head -10)
    fi

    local USB_LINES
    USB_LINES=$(grep -iE "UsbDeviceManager.*connect|usb.*attach|usb.*device.*connect|gadget.*connect" \
        "$BR_TXT" 2>/dev/null | grep -viE "bluetooth|charger|charge" | head -8)

    if [ -n "$ADB_LINES" ]; then
        log_output "${R}[!] Actividad ADB detectada en log:${N}"
        echo "$ADB_LINES" | while read -r l; do log_output "${Y}  $l${N}"; done
        ((SUSPICIOUS_COUNT+=4)); FOUND=1
    fi

    if [ -n "$USB_LINES" ]; then
        log_output "${Y}[!] Dispositivo USB conectado:${N}"
        echo "$USB_LINES" | while read -r l; do log_output "${W}  $l${N}"; done
        # USB solo puede ser carga — sospechoso si aparece junto con ADB
        [ $FOUND -eq 1 ] && ((SUSPICIOUS_COUNT+=2))
        FOUND=1
    fi

    if [ -n "$ADB_LINES" ] && [ -n "$USB_LINES" ]; then
        log_output "${R}[!] COMBINACIÓN ADB + USB física — alta probabilidad de herramienta de replay${N}"
        ((SUSPICIOUS_COUNT+=3))
    fi

    [ $FOUND -eq 0 ] && log_output "${G}[✓] Sin conexiones ADB/USB recientes detectadas${N}"
    log_output ""
}
...
# ═══════════════════════════════════════════════════════════════
br_check_tcp_remote() {
    sec_hdr "CONEXIONES TCP — PANELES REMOTOS"
    local FOUND=0

    # /proc/net/tcp en el bugreport: IPs en little-endian hex
    # Columnas: sl  local_addr  rem_addr  state  ...
    # State 01 = ESTABLISHED
    local TCP_RAW
    TCP_RAW=$(awk '/\/proc\/net\/tcp/{found=1; next} found && /^[[:space:]]*[0-9]+:/{print} found && /^-{3}/{exit}' \
        "$BR_TXT" 2>/dev/null | head -200)

    if [ -n "$TCP_RAW" ]; then
        local REMOTE_CONNS
        REMOTE_CONNS=$(echo "$TCP_RAW" | awk '
            $4 == "01" {
                split($3, r, ":")
                # Decodificar IP little-endian
                hex = r[1]
                d4 = strtonum("0x" substr(hex,1,2))
                d3 = strtonum("0x" substr(hex,3,2))
                d2 = strtonum("0x" substr(hex,5,2))
                d1 = strtonum("0x" substr(hex,7,2))
                ip = d1"."d2"."d3"."d4
                port = strtonum("0x" r[2])
                # Ignorar loopback, wildcard, RFC1918 en puertos bajos
                if (d1 != 127 && d1 != 0 && port > 1024) {
                    # Marcar RFC1918 (LAN) vs internet
                    if (d1==10 || (d1==172 && d2>=16 && d2<=31) || (d1==192 && d2==168))
                        tipo = "LAN"
                    else
                        tipo = "INTERNET"
                    printf "  %s  IP: %-18s  Puerto: %d\n", tipo, ip, port
                }
            }
        ' 2>/dev/null | sort -u | head -15)

        if [ -n "$REMOTE_CONNS" ]; then
            log_output "${R}[!] Conexiones TCP ESTABLISHED a IPs externas:${N}"
            echo "$REMOTE_CONNS" | while read -r l; do
                echo "$l" | grep -q "INTERNET" \
                    && log_output "${R}  $l${N}" \
                    || log_output "${Y}  $l${N}"
            done
            echo "$REMOTE_CONNS" | grep -q "INTERNET" && ((SUSPICIOUS_COUNT+=5))
            echo "$REMOTE_CONNS" | grep -q "LAN"      && ((SUSPICIOUS_COUNT+=3))
            FOUND=1
        fi
    fi

    # Fallback: buscar strings de panel remoto en logcat
    local PANEL_HITS
    PANEL_HITS=$(grep -iE "tcp.*accept|socket.*bind.*[0-9]{4,5}|remote.*panel|panel.*connect" \
        "$BR_TXT" 2>/dev/null \
        | grep -viE "wifi|bluetooth|system_server|NetworkMonitor" \
        | head -5)
    if [ -n "$PANEL_HITS" ]; then
        log_output "${Y}[!] Referencias a sockets/paneles remotos en log:${N}"
        echo "$PANEL_HITS" | while read -r l; do log_output "${W}  $l${N}"; done
        ((SUSPICIOUS_COUNT+=2)); FOUND=1
    fi

    [ $FOUND -eq 0 ] && log_output "${G}[✓] Sin conexiones TCP sospechosas detectadas${N}"
    log_output ""
}
...
_br_run_analysis() {
    ...
    # ── Info del dispositivo (encabezado) ─────────────────────────────────────
    br_get_device_info
    br_show_device_header
