...
_br_sec() {
    local pattern="$1"
    awk "/------ ${pattern}/,/^------/" "$BR_TXT" 2>/dev/null | grep -v "^------"
}
...
br_check_tombstones() {
    ...
    if [ -d "$TOMB_DIR" ]; then
        for tomb in "$TOMB_DIR"/tombstone_*; do
            [ -f "$tomb" ] || continue
            local SUSP
            SUSP=$(grep -iE "frida|inject|xposed|cheat|hack|zygisk|lsposed|libmod" "$tomb" 2>/dev/null | head -3)
            if [ -n "$SUSP" ]; then
        ...
        if [ -n "$TOMB_SEC" ]; then
            local SUSP
            SUSP=$(echo "$TOMB_SEC" | grep -iE "frida|inject|xposed|cheat|zygisk|libmod" | head -5)
            if [ -n "$SUSP" ]; then
...
_br_run_analysis() {
    ...
    BR_DIR=$(mktemp -d /tmp/unknown_br_XXXXXX)
    ...
    BR_TXT=$(find "$BR_DIR" -maxdepth 2 -name "bugreport*.txt" ! -path "*/FS/*" | head -1)
    if [ -z "$BR_TXT" ]; then
        log_output "${R}[!] No se encontró bugreport.txt en el zip.${N}"
        rm -rf "$BR_DIR"; sleep 2; bugreport_menu; return
    fi
...
_br_pick_file() {
    ...
    echo ""
    if [ ${#FOUND[@]} -gt 0 ]; then
        sec_hdr "BUGREPORTS EN DESCARGAS"
        local i=1
        for f in "${FOUND[@]}"; do
            local SIZE DATE_STR
            SIZE=$(du -sh "$f" 2>/dev/null | cut -f1)
            DATE_STR=$(date -r "$f" "+%d/%m/%Y %H:%M" 2>/dev/null)
            echo -e "${Y}  [$i]${W} $(basename "$f")${N}"
            echo -e "${B}      Tamaño: ${W}$SIZE  ${B}Fecha: ${W}$DATE_STR${N}"
            echo ""
            ((i++))
        done
    else
        echo -e "${Y}[!] No se encontraron bugreports en Descargas.${N}"
        echo -e "${W}    Descargá el .zip desde Drive, Telegram, etc. a Descargas.${N}"
        echo ""
    fi

    echo -e "${C}  [R]${W} Ingresar ruta manualmente${N}"
    echo -e "${R}  [V]${W} Volver${N}"
    echo ""
    echo -ne "${Y}  Seleccioná (1-${#FOUND[@]} / R / V): ${N}"
    read -r SEL

    case "${SEL^^}" in
        V) return 1 ;;
        R)
            echo -ne "${Y}  Ruta completa del archivo: ${N}"
            read -r RUTA_MANUAL
            RUTA_MANUAL=$(echo "$RUTA_MANUAL" | tr -d "'\"\r")
            if [ -f "$RUTA_MANUAL" ] && [ -s "$RUTA_MANUAL" ]; then
                echo "$RUTA_MANUAL"
                return 0
            fi
            echo -e "${R}[!] Archivo no encontrado o vacío.${N}"
            return 1
            ;;
        *)
...
bugreport_menu() {
    ...
    case "${_br_opc^^}" in
        A)
            ...
            local BR_OUT="/sdcard/Download/bugreport_$(date +%Y%m%d_%H%M%S).zip"
            echo ""
