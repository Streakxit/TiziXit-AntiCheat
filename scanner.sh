...
br_check_root() {
    ...
    if [ -n "$ROOT_PATHS" ]; then
        log_output "${R}[!] Paths de root detectados:${N}"
...
br_check_adb_usb() {
    ...
    # ── Hora de generación del bugreport (del nombre del zip) ─────────────────
    local BR_FNAME GEN_H=-1 GEN_M=-1 GEN_S=-1
    BR_FNAME=$(basename "${ZIP_PATH:-$BR_TXT}")
    if [[ "$BR_FNAME" =~ [0-9]{4}-[0-9]{2}-[0-9]{2}-([0-9]{2})-([0-9]{2})-([0-9]{2}) ]]; then
        GEN_H=$(( 10#${BASH_REMATCH[1]} ))
        GEN_M=$(( 10#${BASH_REMATCH[2]} ))
        GEN_S=$(( 10#${BASH_REMATCH[3]} ))
    fi
    local GEN_SECS=$(( GEN_H * 3600 + GEN_M * 60 + GEN_S ))

    # Clasifica un evento: SCANNER si ocurrió ≤10 min antes de la generación, PREVIO si antes
    _adb_tipo() {
        local line="$1"
        local ts h m s ev_secs diff
        ts=$(echo "$line" | grep -oE "[0-9]{2}:[0-9]{2}:[0-9]{2}" | head -1)
        [ -z "$ts" ] || [ "$GEN_H" -eq -1 ] && { echo "DESCONOCIDO"; return; }
        h=$(( 10#$(echo "$ts" | cut -d: -f1) ))
        m=$(( 10#$(echo "$ts" | cut -d: -f2) ))
        s=$(( 10#$(echo "$ts" | cut -d: -f3) ))
        ev_secs=$(( h * 3600 + m * 60 + s ))
        diff=$(( GEN_SECS - ev_secs ))
        [ $diff -lt 0 ] && diff=$(( diff + 86400 ))
        [ $diff -le 600 ] && echo "SCANNER" || echo "PREVIO"
    }

    # ── ro.adb.secure ─────────────────────────────────────────────────────────
    local ADB_SECURE
    ADB_SECURE=$(echo "$PROPS" | grep -E "\[ro\.adb\.secure\]" | sed 's/.*\]: \[//;s/\]//')
    [ "$ADB_SECURE" = "0" ] && log_output "${Y}[*] ro.adb.secure = 0 (sin autenticación ADB)${N}"

    # ── persist.sys.usb.config ────────────────────────────────────────────────
    local USB_CFG
    USB_CFG=$(echo "$PROPS" | grep -E "\[persist\.sys\.usb\.config\]" | sed 's/.*\]: \[//;s/\]//')
    echo "$USB_CFG" | grep -q "adb" && \
        log_output "${Y}[*] persist.sys.usb.config = $USB_CFG${N}"

    # ── 3 eventos más recientes de conexión/desconexión ───────────────────────
    local EVENTS
    EVENTS=$(grep -E \
        "UsbDeviceManager.*USB state|adbd.*starting|usb_debugging is enabled|Device is compromised|adbd.*offline|adbd.*disconnect" \
        "$BR_TXT" 2>/dev/null | tail -3)

    if [ -n "$EVENTS" ]; then
        log_output ""
        log_output "${W}  Últimas 3 conexiones/desconexiones ADB:${N}"
        local event_suspicious=0
        while IFS= read -r l; do
            local ts tipo evento color
            ts=$(echo "$l" | grep -oE "[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+" | head -1)
            [ -z "$ts" ] && ts="sin timestamp"
            tipo=$(_adb_tipo "$l")
            echo "$l" | grep -qiE "starting|ADB|usb_debugging|compromised" \
                && evento="CONEXION" || evento="DESCONEXION"
            if [ "$tipo" = "SCANNER" ]; then
                log_output "${B}  [$ts]  $evento  —  del scanner, ignorar${N}"
            elif [ "$tipo" = "PREVIO" ]; then
                log_output "${R}  [$ts]  $evento  —  PREVIO AL SCANNER — APLICAR W.O.${N}"
                event_suspicious=1
            else
                log_output "${Y}  [$ts]  $evento  —  hora no determinada${N}"
            fi
        done <<< "$EVENTS"
        [ $event_suspicious -eq 1 ] && { ((SUSPICIOUS_COUNT+=4)); FOUND=1; }
    fi

    # ── Puerto 5555 — ADB over network ────────────────────────────────────────
    local ADB_TCP
    ADB_TCP=$(grep -E ":::5555|0\.0\.0\.0:5555|:5555.*adbd|adbd.*5555" \
        "$BR_TXT" 2>/dev/null | head -3)
    if [ -n "$ADB_TCP" ]; then
        log_output "${R}[!] ADB escuchando en red (puerto 5555):${N}"
        echo "$ADB_TCP" | while IFS= read -r l; do log_output "${Y}  $l${N}"; done
        ((SUSPICIOUS_COUNT+=4)); FOUND=1
    fi

    [ $FOUND -eq 0 ] && log_output "${G}[✓] Sin conexiones ADB/USB sospechosas${N}"
    log_output ""
}
...
# ═══════════════════════════════════════════════════════════════
br_check_tcp_remote() {
    sec_hdr "CONEXIONES TCP — REMOTE"
    local FOUND=0

    local TCP_RAW
    TCP_RAW=$(awk '/\/proc\/net\/tcp/{found=1; next} found && /^[[:space:]]*[0-9]+:/{print} found && /^-{6}/{exit}' \
    ...
    [ $FOUND -eq 0 ] && log_output "${G}[✓] Sin conexiones TCP sospechosas${N}"
    log_output ""
}
...
br_check_props() {
    ...
    [ -n "$BOOT" ] && [ "$BOOT" != "green" ] && { log_output "${Y}[!] Boot state no es GREEN${N}"; ((SUSPICIOUS_COUNT++)); FOUND=1; }
