...
remote_review_menu() {
    ...
    echo_hdr "REVISION REMOTA DE PANTALLA  (BETA)" "$C"
    echo ""
    echo -e "${Y}$(_hl $COLS ═)${N}"
    echo -e "${Y}!  AVISO — LEER ANTES DE CONTINUAR${N}"
    echo -e "${Y}$(_hl $COLS ─)${N}"
    echo ""
    echo -e "${W}  El SS va a ver y controlar la pantalla del dispositivo${N}"
    echo -e "${W}  en tiempo real — igual que AnyDesk.${N}"
    echo ""
    echo -e "${W}  El usuario ve TODO lo que el SS hace en su pantalla.${N}"
    echo ""
    echo -e "${C}  Para cortar el acceso en cualquier momento:${N}"
    echo -e "${W}    Ajustes → Depuración Inalámbrica → Desactivar${N}"
    echo ""
    echo -e "${Y}$(_hl $COLS ═)${N}"
    echo ""
    ...
    echo ""
    echo -e "${G}$(_hl $COLS ═)${N}"
    echo -e "${G}  REVISION REMOTA — SESION INICIADA${N}"
    echo -e "${G}$(_hl $COLS ─)${N}"
    echo ""
    echo -e "${W}  Abrí el navegador del teléfono y entrá a:${N}"
    echo -e "${C}  http://${MY_IP}:8888${N}"
    echo ""
    echo -e "${W}  Código de acceso (decíselo al SS verbalmente):${N}"
    echo -e "${Y}  $(_bc "$CODE" $(( COLS - 2 )))${N}"
    echo ""
    echo -e "${W}  Para cortar: Ajustes → Depuración Inalámbrica → Desactivar${N}"
    echo ""
    echo -e "${G}$(_hl $COLS ═)${N}"
    echo ""
