...
bugreport_menu() {
    ...
    echo -e "${W}  4. Compartir el .zip con el SS (Drive, Telegram, etc.)${N}"
    echo ""
    echo -e "${Y}$(_hl $COLS ─)${N}"
    echo ""
    echo -e "${Y}[A]${W} Capturar bugreport por ADB (dispositivo conectado)${N}"
    echo -e "${Y}[M]${W} Analizar bugreport ya descargado (ingresar ruta del .zip)${N}"
    ...
    case "${_br_opc^^}" in
        ...
        M)
            echo ""
            echo -ne "${Y}  Ruta al .zip: ${N}"
            read -r BR_PATH
            BR_PATH=$(echo "$BR_PATH" | tr -d '"' | sed "s|^~|$HOME|")
            if [ ! -f "$BR_PATH" ]; then
                echo -e "${R}[!] Archivo no encontrado: $BR_PATH${N}"
                sleep 2; bugreport_menu; return
            fi
            _br_run_analysis "$BR_PATH"
            ;;
        V|*) main_menu ;;
    esac
}
