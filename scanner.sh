...
br_check_adb_usb() {
    ...
    if echo "$USB_CFG" | grep -q "adb"; then
        log_output "${Y}[*] ADB habilitado en USB: persist.sys.usb.config = $USB_CFG${N}"
        log_output "${B}    (puede ser del propio scanner — verificar con los eventos siguientes)${N}"
        FOUND=1
    fi

    # ── UsbDeviceManager — estado USB con ADB (con timestamp) ────────────────
    local USB_STATE
    USB_STATE=$(grep -E "UsbDeviceManager.*USB state.*ADB|UsbDeviceManager.*ADB" \
        "$BR_TXT" 2>/dev/null | head -3)
    if [ -n "$USB_STATE" ]; then
        log_output "${R}[!] USB con ADB activo:${N}"
        echo "$USB_STATE" | while IFS= read -r l; do
            local TS
            TS=$(echo "$l" | grep -oE "[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+" | head -1)
            [ -n "$TS" ] && log_output "${Y}  [$TS] $l${N}" || log_output "${Y}  $l${N}"
        done
        ((SUSPICIOUS_COUNT+=2)); FOUND=1
    fi

    # ── adbd — mensajes específicos (con timestamp) ───────────────────────────
    local ADBD_MSG
    ADBD_MSG=$(grep -E "adbd.*starting|usb_debugging is enabled|Device is compromised" \
        "$BR_TXT" 2>/dev/null | head -3)
    if [ -n "$ADBD_MSG" ]; then
        log_output "${R}[!] adbd activo en el dispositivo:${N}"
        echo "$ADBD_MSG" | while IFS= read -r l; do
            local TS
            TS=$(echo "$l" | grep -oE "[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]+" | head -1)
            [ -n "$TS" ] && log_output "${Y}  [$TS] $l${N}" || log_output "${Y}  $l${N}"
        done
